...
namespace PowerFolder.Logging
{
	...
	public class LogMethods //: System.IDisposable
	{
		...
		public void LogThis(string logtext,eloglevel loglevel, elogprefix logprefix)
		{
			if (m_loglevel >= loglevel)
			{
				...
				{
					case elogprefix.dt:
						logtext = dt.ToString("yyyy.MM.dd") + "-" + dt.ToString("h.mm.ss") + ": " + logtext;
						break;
    ...
    public class PowerFolderLogProfile
    {
        public static void initLogProfile()
        {
            Log.UseSensibleDefaults("ErrorLog", getLoggingFolderPath(), eloglevel.error);
            Log.Profile = elogprofile.primary;
            Log.ProcessName = "PowerFolder Outlook";
        ...
        private static string getLoggingFolderPath()
        {
            String path = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + "\\PowerFolder\\";
